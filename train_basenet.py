...
'''
argparse 的使用,用来处理命令行的参数，也有提示之类的功能
'''
parser=argparse.ArgumentParser()  # 创建对象
parser.add_argument('--exp',type=str,required=True,help='Name of Experiment')  # 调用函数加入参数
parser.add_argument('--data_dir_images',type=str,required=True,help='Path to shapenet rendered images')
FLAGS=parser.parse_args()  # 调用函数解析，会返回一个有2个属性的对象
print(FLAGS)

exp_name=FLAGS.exp
data_dir_images=FLAGS.data_dir_images
